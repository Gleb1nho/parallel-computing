...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        std::ifstream input_data("initial-data.txt");
        ...
        {
            // На маке собралось со 2 вариантом, на линуксе и с первым норм
            // int *loc_size = new int[2]{height / size + 2, width};
            int loc_size[2] = { height / size + 2, width };
        ...
        for (int gen = 1; gen <= COUNT_STEPS_EVOLUTION; gen++)
        {
            ...
            if (gen == 1)
            {
                ...
                cout << "Time for iteration: " << Time_work_iter << endl;
            }
        }
        for (int i = 0; i < loc_height; i++)
            for (int j = 0; j < width; j++)
        ...
        std::ofstream out("result.txt", std::ofstream::out | std::ofstream::trunc);
        if (out.is_open())
        {
    ...
    if (rank == 0)
    {
        cout << "Total execution time: " << Time_work << endl;
    }
    MPI_Finalize();
    return 0;
}